...
class TaxiStatistics():
    ...
    def find_shift_1_taxi(self,taxi_no,i):#
        ...
        try:
            targetLine = "";  
            i=0
            with codecs.open('F:\\TaxiData\\shift_grid\\2shift_grid_'+taxi_no+'.csv','rb',encoding='gbk') as f : # 使用with读文件 
                for linetemp in f:
                    i+=1
                    if i==1:
                        targetLine=linetemp
                        break
            linelist=targetLine.split(",")
            ...
            print(e)
            f=open('F:\\TaxiData\\shift_grid\\shift_top\\shift_grid_log.txt','a') 
            f.write(taxi_no+"exception!")
            f.write("\n")
            f.close()  
            return
    ...
    def find_shift_all_taxi(self):#追踪所有的出租车，一共11728辆，将其上车、下车的次数加入相应的区域格子，基于内存和文件
        ...
        f = open("taxi\\result1All_20141017.txt") 
        writer=csv.writer(file('F:\\TaxiData\\shift_grid\\shift_top\\statistic_grid_0101_all_shift_top_finally.csv','ab'))
        line = f.readline()  
        i=0          
        while line!="\n" and line:
            i+=1
